class Graph {
  ...
  addEdge(xVertex, yVertex) {
    this.edges[xVertex].push(yVertex)
    this.edges[yVertex].push(xVertex)
    this.numberOfEdges++
  }

  removeEdge(xVertex, yVertex) {
    const xIdx = (this.edges[xVertex] || []).indexOf(yVertex)
    const yIdx = (this.edges[yVertex] || []).indexOf(xVertex)
    if (~xIdx) {
      this.edges[xVertex].splice(xIdx, 1)
      this.numberOfEdges--
    }
    if (~yIdx) {
      this.edges[yVertex].splice(yIdx, 1)
    }
  }
