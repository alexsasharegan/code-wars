...
function getPINs(observed) {
  ...
  const combos = []
  function permutate(idx = 0, curPin = "") {
    for (const p of comboMap[digits[idx]]) {
      if (idx === lastDigit) {
        combos.push(curPin + p)
      } else {
        permutate(idx + 1, curPin + p)
      }
    }
    return combos
  }
...
for (var pin in expectations) {
  const tested = getPINs(pin).sort()
  if (JSON.stringify(tested) !== JSON.stringify(expectations[pin].sort())) {
    console.error("PIN: " + pin, "TESTED: " + tested)
  }
}
