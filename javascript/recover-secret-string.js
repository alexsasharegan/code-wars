...
function recoverSecret(triplets) {
  let chars = Array.from(new Set(Array.prototype.concat.apply([], triplets)))
  Array.from({ length: triplets[0].length }).forEach(() => {
    triplets.forEach(triplet => {
      triplet.forEach((char, i) => {
        let charIdx = chars.indexOf(char)
        for (let j = i + 1; j < triplet.length; j++) {
          const refChar = triplet[j]
          const refIdx = chars.indexOf(refChar)
          if (refIdx < charIdx) {
            ;[chars[refIdx], chars[charIdx]] = [char, refChar]
            charIdx = refIdx
          }
        }
      })
